...
function Cart() {
  ...
  async function enviarPedido() {
    ...
    const pedidoParaSalvar = {
      ...
      itens: cartItems.map((i) => ({ produto: i.nome, nome: i.nome, qtd: i.qtd, preco: i.preco })),
          ...
          {cartItems.length === 0 ? (
            ...
            <>
              <div className="lista-produtos" style={{ flexGrow: 1, overflowY: "auto", minHeight: "150px", marginBottom: "15px" }}>
                {cartItems.map((item) => (
                  <ProdutoCart key={item.id} id={item.id} nome={item.nome} preco={item.preco} foto={item.foto} qtd={item.qtd} adicionais={item.adicionais} />
                ))}
              </div>
      ...
      {showModal && (
        <div className="modal-overlay" style={{ zIndex: 9999, position: "fixed", top: 0, left: 0, width: "100%", height: "100%", backgroundColor: "rgba(0,0,0,0.5)", display: "flex", alignItems: "center", justifyContent: "center" }}>
          <div className="modal-box" style={{ background: "white", padding: "25px", borderRadius: "12px", maxWidth: "300px", textAlign: "center" }}>
            <h2 style={{ marginBottom: "10px" }}>Confirmar Pedido?</h2>
            
            {/* NOVO TEXTO ADICIONADO ABAIXO */}
            <p style={{ fontSize: "0.9rem", color: "#666", lineHeight: "1.4" }}>
              Seu pedido será enviado para o WhatsApp da loja para produção.
            </p>

            <div style={{ display: "flex", gap: "10px", marginTop: "20px" }}>
              <button
                onClick={() => { setShowModal(false); setShow(true); }}
                className="cancel-btn"
                style={{ flex: 1, padding: "10px", cursor: "pointer", border: "1px solid #ccc", borderRadius: "8px", background: "white" }}
              >
                REVISAR
              </button>
              <button
